...
\subsubsection{Хорошо обусловленная ($\kappa = 1$)}
\[
    f(x, y) = x^2 + y^2,
    \quad
...
\subsubsection{Плохо обусловленная ($\kappa = 100$)}
\[
...
\paragraph{Вывод.} Оптимальный шаг $\alpha = 0.5$ обеспечивает сходимость за 1 итерацию.
...
0.0195  & 971    & $4.9 \times 10^{-17}$ & \checkmark \\
0.0199  & 2361   & $4.9 \times 10^{-19}$ & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Вывод.} При $\alpha=0.0199$ (вблизи границы устойчивости $2/L=0.02$) итераций больше, чем при $\alpha=0.0195$: метод начинает осциллировать вдоль оси $x$.
...
Розенброк & 0.001 & 43564 & $1.25\times10^{-16}$ & $(1.0000,1.0000)$ & \checkmark \\
\midrule
Экли & 0.100 & — (локал.) & 3.72 & $(-0.33, 0.44)$ & $\times$ \\
Экли & 0.010 & 39 & 3.57 & $(0.97, 0.97)$ & \checkmark \\
Экли & 0.001 & 480 & 3.57 & $(0.97, 0.97)$ & \checkmark \\
\midrule
Химмельблау & 0.100 & — (inf) & $\infty$ & diverged & $\times$ \\
Химмельблау & 0.010 & 77 & $1.94\times10^{-18}$ & $(3.0000,2.0000)$ & \checkmark \\
Химмельблау & 0.001 & 849 & $1.89\times10^{-18}$ & $(3.0000,2.0000)$ & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Розенброк, $\alpha \geq 0.01$:} расходимость.
    \item \textbf{Экли, $\alpha=0.1$:} метод остановился в локальном минимуме ($f \approx 3.72 \neq 0$).
    \item \textbf{Химмельблау, $\alpha=0.1$:} расходимость.
\end{itemize}
...
\paragraph{Вывод.} Армихо требует $\sim$870 итераций при $\eps=10^{-8}$, Вольфе — всего \textbf{3 итерации}.
...
\paragraph{Вывод.} На $\kappa=1$ метод сходится за 1 итерацию. На $\kappa=100$ траектория зигзагообразна.
...
\begin{enumerate}
    \item \textbf{Постоянный шаг.} Прост в реализации, но требует ручного подбора $\alpha \in (0, 2/L)$.

    \item \textbf{Число обусловленности $\kappa$} — главный параметр.

    \item \textbf{Армихо vs Вольфе.} Вольфе выигрывает в разы на плохо обусловленных задачах.

    \item \textbf{Наискорейший спуск.} На квадратиках с $\kappa=1$ — оптимален (1 итерация).

    \item \textbf{Многоэкстремальные функции.} Градиентные методы первого порядка не гарантируют глобального минимума.
\end{enumerate}
